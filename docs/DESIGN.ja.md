...
Codex側の起動と待機は[SKILL.mdのRun AGY and wait](../payload/.agents/skills/explore-solve/SKILL.md#run-agy-and-wait)の一つのセルで行います。
内側のセッション待機をモデルへ返さず、外側のセルには長い待機時間を指定します。
ホストが途中でセルを返した場合は、同じセルを利用可能な長い時間で待ち直します。
ホスト側の返却を無効化する機能や、別の完了通知サービスは追加していません。
